-- =============================================
-- COMPREHENSIVE ENHANCED SAMPLE DATA v4.0
...
-- * ~100 shipments with complete lifecycle data (via separate script)
-- * 200+ laptops (diverse brands and configurations)
...
-- =============================================
-- LAPTOPS (200+ comprehensive inventory with AUTO-GENERATED SKUs)
-- =============================================
...
-- =============================================
-- ADDITIONAL LAPTOPS (Generated programmatically for ~100 shipments)
-- =============================================
-- Generate ~120 additional laptops to support bulk shipments
DO $$
DECLARE
    i INT;
    serial_num TEXT;
    ...
    status_val TEXT;
    client_id INT;
    brands TEXT[] := ARRAY['Dell', 'Lenovo', 'HP', 'Apple'];
    models TEXT[] := ARRAY['XPS 15 9520', 'ThinkPad X1 Carbon Gen 10', 'ZBook Studio G9', 'MacBook Pro 16" M2 Pro'];
    cpus TEXT[] := ARRAY['Intel Core i7-12700H', 'Intel Core i7-1270P', 'Intel Core i9-12900H', 'Apple M2 Pro'];
    rams TEXT[] := ARRAY['32GB', '64GB', '32GB', '32GB'];
    ssds TEXT[] := ARRAY['1TB', '2TB', '2TB', '1TB'];
    skus TEXT[] := ARRAY['C.NOT.0I7.032.1T', 'C.NOT.0I7.032.1T', 'C.NOT.0I9.064.2T', 'C.MAC.MP2.032.1T'];
    statuses TEXT[] := ARRAY['available', 'at_warehouse', 'available', 'available'];
    brand_idx INT;
BEGIN
    FOR i IN 1..120 LOOP
        -- Cycle through brands/models
        brand_idx := 1 + ((i - 1) % 4);
        brand_name := brands[brand_idx];
        model_name := models[brand_idx];
        cpu_name := cpus[brand_idx];
        ram_val := rams[brand_idx];
        ssd_val := ssds[brand_idx];
        sku_val := skus[brand_idx];
        status_val := statuses[brand_idx];
        
        -- Distribute across clients (1-15)
        client_id := 1 + ((i - 1) % 15);
        
        -- Generate serial number
        IF brand_name = 'Dell' THEN
            serial_num := 'DELL-XPS-' || LPAD((i + 100)::TEXT, 3, '0');
        ELSIF brand_name = 'Lenovo' THEN
            serial_num := 'LENOVO-X1C-' || LPAD((i + 100)::TEXT, 3, '0');
        ELSIF brand_name = 'HP' THEN
            serial_num := 'HP-ZBOOK-' || LPAD((i + 100)::TEXT, 3, '0');
        ELSE
            serial_num := 'APPLE-MBP-' || LPAD((i + 100)::TEXT, 3, '0');
        END IF;
        
        -- Insert laptop
        INSERT INTO laptops (serial_number, sku, brand, model, cpu, ram_gb, ssd_gb, status, client_company_id, created_at, updated_at)
        VALUES (
            serial_num,
            sku_val,
            brand_name,
            ...
            status_val,
            client_id,
            NOW() - INTERVAL '6 months' + (i || ' days')::INTERVAL,
            NOW() - INTERVAL '6 months' + (i || ' days')::INTERVAL
        );
    END LOOP;
END $$;
...
SELECT '========================================' AS separator;
SELECT 'COMPREHENSIVE SAMPLE DATA v4.0 LOADED!' AS message;
SELECT '========================================' AS separator;
SELECT '' AS blank;
